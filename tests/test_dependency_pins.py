...
def test_pyproject_pins_git_sources_by_tag_or_rev_not_main() -> None:
    ...
    assert sources["app-factory"]["tag"] == "v0.6.11"
    assert sources["my-auth"]["tag"] == "v0.4.6"
...
def test_readme_documents_tag_and_rev_pins_instead_of_main() -> None:
    ...
    assert "app-factory" in readme and "v0.6.11" in readme
    assert "my-auth" in readme and "v0.4.6" in readme
